...
egarch.fit <- ugarchfit(spec=model, data=sp500ret)

model <- ugarchspec(variance.model=list(model = "csGARCH", garchOrder = c(2, 1)),
                    mean.model=list(armaOrder = c(0, 0), include.mean = FALSE),
                    distribution.model="nig")

cs.fit <- ugarchfit(spec=model, data=sp500ret)
